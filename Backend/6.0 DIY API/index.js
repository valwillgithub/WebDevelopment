...
app.get('/jokes/:id', (req, res) => {
	// const id = parseInt(req.query.id) // how to access Query parameters
	const id = parseInt(req.params.id); // how to access URL/Path parameters
...
app.get('/filter', (req, res) => {
	const jt = req.query.type; // how to access Query parameters
	console.log('req.query.type => ', req.query.type);
	const foundJoke = jokes.filter((joke) => {
		return joke.jokeType === jt;
	});
	res.json(foundJoke);
});
...
app.delete('/jokes/:id', (req, res) => {
	...
	if (searchIndex > -1) {
		...
		res
			.status(200)
			.json({ Success: `Joke with id: ${id} successfully deleted.` });
	} else {
...
app.delete('/all', (req, res) => {
	...
	if (userKey === masterKey) {
		jokes = [];
		res.status(200).json({ Success: 'All jokes successfully deleted.' });
	} else {
